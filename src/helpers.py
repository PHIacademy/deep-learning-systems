...
def compute_mean_and_std_concurrent():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        for mean_value in executor.map(
            compute_mean,
            (images for (images, _) in tqdm(dl, total=len(ds), desc="Computing mean", ncols=80)),
        ):
            mean += mean_value
    ...
    npix = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        for (var_value, npix_value) in executor.map(
            compute_var_and_npix,
            (images for (images, _) in tqdm(dl, total=len(ds), desc="Computing std", ncols=80)),
        ):
            var += var_value
            npix += npix_value
...
def plot_confusion_matrix(pred, truth):
    ...
    with sns.plotting_context("notebook"):
        ...
        sns.heatmap(
            confusion_matrix, annot=True, ax=sub, linewidths=0.5, linecolor="lightgray", cbar=False
        )
