...
def get_data_loaders(
    ...
    # Create 3 sets of data transforms: one for the training dataset (with
    # data augmentation), one for the validation dataset (no augmentation)
    # and one for the test set (no augmentation). Images are resized to 256
    # then cropped to 224.
    data_transforms = {
    ...
    # from the same folder, but apply the validation transforms
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    # Convert from BGR (the format used by pytorch) to RGB (the format
